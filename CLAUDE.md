...
5. **Schedule** (`scheduler.py`) — a lifespan task sweeps all 14 ROIs every `SCHEDULER_INTERVAL_SECONDS` and analyzes each new usable pass once, under a `PU_MONTHLY_CEILING`. Nobody requests imagery; the admin POST survives only as an ops escape hatch.
...
- Coded: AIS ingestion, full pipeline (SAR fetch → YOLOv8 → fusion), react-leaflet UI, 14-region registry with the fused/survey split, SAR overview retention + map overlay, and the automatic analysis scheduler (`scheduler.py`) with its PU ledger and ceiling.
- **Pipeline is proven end-to-end.** The scheduler ran unattended on 2026-07-27 and analyzed 4 survey regions (`hormuz_strait`, `fujairah_anchorage`, `kharg_island`, plus an interrupted `eopl_tompok_utara`/`kerch_strait`) for 274 PU total. CDSE creds and `models/sar_ship.pt` are present and working — the old "blocked on 3 user inputs" note was stale.
- **No fused region has been analyzed yet.** All six were refused with "no scene falls inside the AIS buffer" because AIS ingest had only just restarted. Let AIS run several hours, then the first fused analysis (`singapore_strait`, ~55 PU) will happen on its own.
...
- Next: watch one automatic fused run land, then re-probe all 14 `ais_bbox`es.
