...
1. **AIS ingestion** (`ais.py`, `ingest.py`) — AISStream WebSocket filtered by the union of every ROI's `ais_bbox`; positions stream continuously into PostGIS (persistent connection, no polling). Short retention (`AIS_RETENTION_DAYS`, default 5) since correlation only needs AIS bracketing each ~daily pass. Switching the frontend's ROI is a pure view filter — ingestion never narrows.
